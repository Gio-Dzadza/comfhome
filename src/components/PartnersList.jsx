...
export default function PartnersList({ updateList, setUpdateList }) {
    ...
    //mui
    const [muiFilteredPartnersCount, setMuiFilteredPartnersCount] = useState(0);


    // Function to export data to Excel
    const exportToExcel = () => {
    // Convert the user data into an array of arrays
        const dataToExport = partnersList.map((par) => [
            par.name,
            par.description, par.link,
            formatDate(par.created_at ? par.created_at : ''),
            formatDate(par.changed_at ? par.changed_at : ''),
        ]);

        // Create a new workbook and worksheet
        const workbook = XLSX.utils.book_new();
        const worksheet = XLSX.utils.aoa_to_sheet([
            ['სახელი', 'აღწერა', 'ლინკი', 'შექმნილია', 'ცვლილება განხორციელდა'],
            ...dataToExport,
        ]);
    ...
    const fetchData = useCallback(async (...signal) => {
        ...
        try {
            const response = await Axios.get(
                'http://localhost:3001/api/partners/get/partners',
                {
                    ...signal,
                    headers: {
                    ...
                    if(!response.data.auth){
                        ...
                        setPartnersList(response.data.result);
            ...
            setIsPending(false);
            setError("Couldn't fetch the data from partners");
        }
    },[context]);
    ...
    const handleDelete = async(id)=>{
        try {
            ...
            await Axios.delete(`http://localhost:3001/api/partners/partners/delete/${id}`, {
                headers: {
                    "x-access-token": context.userAccessToken, // Include the token in the headers
                },
            }).then((response)=>{
                ...
                if(response.data.deleted){
                    console.log(id + ' ' + response.data.message);
                    // Update the UI
                    const updatedPartners = partnersList.filter((par)=> par.id !== id);
    ...
    const columns = [
            ...
            renderCell: (params) => (
                params.row.image ? (
                    <div className='dep-image-container'>
                        <img
                            src={`http://localhost:3001/uploads/partners/${params.row.id}/${encodeURIComponent(params.row.image.trim())}`}
                            alt="pic"
                            className='dep-image'
                        />
                    </div>
        ...
        { field: 'link', headerName: 'ლინკი', headerClassName: 'column-headers', flex: 1, },
    ...
    const filteredRows = partnersList && partnersList.filter((dep) => {
...
return (
    <div className='list'>
        ...
        {
            selectedPartner && (
                <div className={ selectedPartner ? 'modal-window-show' : 'modal-window' }>
                    <PartnerEditForm
            ...
            !authenticated && (
                <>
                    <DataGrid
                        ...
                        onStateChange={(e) => {
                            setMuiFilteredPartnersCount(e.rowsMeta.positions.length)
                        }}
                        sx={{
                            boxShadow: 2,
                            border: 2,
                    ...
                    <div className='counter-export'>
                        {filteredRows && <h5 className='client-counter'>პარტნიორები: {muiFilteredPartnersCount}</h5>}
